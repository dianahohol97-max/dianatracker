...
const PHASES = [
  ...
  { id:'apr2', label:'April · Week 3-4', color:'#C94C6E', tasks:[
    {id:'t5',n:'VistelaCo — Agent 1: Canva template generation',plannedDays:4,deadline:'2026-04-21',note:'Depends on Canva API approval — 3-7 days wait'},
  ...
  { id:'may1', label:'May · Week 1-2', color:'#4CC98A', tasks:[
    {id:'t9',n:'VistelaCo — Agent 3: Etsy listings via API',plannedDays:4,deadline:'2026-05-09',note:'More complex — 1 week realistic'},
  ...
  { id:'jun2', label:'June · Week 3-4', color:'#4CC9C9', tasks:[
    ...
    {id:'t20',n:'First consulting client via Upwork',plannedDays:0,deadline:'2026-06-30',note:'Ongoing'},
  ]},
...
function isOverdue(ds,done){if(done||!ds)return false;return new Date(ds)<new Date()}

const G='#C9A84C',D1='#0F0F0F',D2='#181818',D3='#222',D4='#2a2a2a',MU='#666'
...
export default function Tracker(){
  ...
  useEffect(()=>{
    try{const s=localStorage.getItem('diana_tracker_v3');if(s)setState(JSON.parse(s))}catch(e){}
  },[])

  function save(s){setState(s);try{localStorage.setItem('diana_tracker_v3',JSON.stringify(s))}catch(e){}}
  ...
  function addSubtask(taskId){
    ...
    if(!text)return
    const existing=state[taskId]?.subtasks||[]
    save({...state,[taskId]:{...state[taskId],subtasks:[...existing,{id:Date.now(),text,done:false,type:'note'}]}})
    setNewSubtask(p=>({...p,[taskId]:''}))
  }

  function addBlocker(taskId){
    const text=newSubtask[taskId]?.trim()
    if(!text)return
    const existing=state[taskId]?.subtasks||[]
    save({...state,[taskId]:{...state[taskId],subtasks:[...existing,{id:Date.now(),text,done:false,type:'blocker'}]}})
    setNewSubtask(p=>({...p,[taskId]:''}))
  }

  function toggleSubtask(taskId,subId){
    const subs=(state[taskId]?.subtasks||[]).map(s=>s.id===subId?{...s,done:!s.done}:s)
    save({...state,[taskId]:{...state[taskId],subtasks:subs}})
  }

  function deleteSubtask(taskId,subId){
    const subs=(state[taskId]?.subtasks||[]).filter(s=>s.id!==subId)
    save({...state,[taskId]:{...state[taskId],subtasks:subs}})
  }
    ...
    <div style={{background:D1,minHeight:'100vh',color:'#E8E8E0',fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',sans-serif",fontSize:15,paddingBottom:60}}>
          ...
          return(
            <div key={ph.id}>
              <div style={{fontSize:10,letterSpacing:'2px',textTransform:'uppercase',color:ph.color,opacity:.8,padding:'14px 20px 8px',display:'flex',alignItems:'center',gap:10}}>
                {ph.label}<span style={{fontSize:10,color:'#444',fontWeight:400}}>{phDone}/{ph.tasks.length}</span>
              </div>
              <div style={{padding:'0 20px'}}>
                {ph.tasks.map(t=>{
                  ...
                  const subtasks=state[t.id]?.subtasks||[]
                  const blockers=subtasks.filter(s=>s.type==='blocker'&&!s.done)

                  return(
                    <div key={t.id} style={{background:D2,borderRadius:12,padding:'13px 16px',marginBottom:7,opacity:isDone?.5:1,borderLeft:'3px solid '+(blockers.length>0?'#C94C4C':ph.color)}}>
                      <div style={{display:'flex',alignItems:'flex-start',gap:12}} onClick={()=>toggleDone(t.id)}>
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          ...
                          {blockers.length>0&&<div style={{fontSize:11,color:'#C94C4C',marginTop:3}}>🚫 {blockers.length} blocker{blockers.length>1?'s':''}</div>}
                          <div style={{display:'flex',alignItems:'center',gap:8,marginTop:6,flexWrap:'wrap'}}>
                            ...
                            {subtasks.length>0&&<span style={{fontSize:11,color:MU}}>📝 {subtasks.length} note{subtasks.length>1?'s':''}</span>}
                          </div>
                        </div>
                        ...
                        <div style={{marginTop:12,paddingTop:12,borderTop:'1px solid #2a2a2a'}} onClick={e=>e.stopPropagation()}>
                          ...
                          {subtasks.length>0&&(
                            <div style={{marginBottom:10}}>
                              {subtasks.map(s=>(
                                <div key={s.id} style={{display:'flex',alignItems:'flex-start',gap:8,padding:'7px 10px',background:D4,borderRadius:8,marginBottom:5,borderLeft:'2px solid '+(s.type==='blocker'?'#C94C4C':'#4CC98A')}}>
                                  <div onClick={()=>toggleSubtask(t.id,s.id)} style={{width:16,height:16,borderRadius:4,border:s.done?'none':'1.5px solid #444',background:s.done?(s.type==='blocker'?'#C94C4C':'#4CC98A'):'transparent',flexShrink:0,marginTop:1,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',fontSize:10,color:'white',fontWeight:700}}>
                                    {s.done?'✓':''}
                                  </div>
                                  <div style={{flex:1,fontSize:12,color:s.done?'#444':'#E8E8E0',textDecoration:s.done?'line-through':'none',lineHeight:1.5}}>
                                    {s.type==='blocker'&&!s.done&&<span style={{color:'#C94C4C',marginRight:4}}>🚫</span>}
                                    {s.text}
                                  </div>
                                  <button onClick={()=>deleteSubtask(t.id,s.id)} style={{background:'none',border:'none',color:'#444',cursor:'pointer',fontSize:13,padding:'0 2px',flexShrink:0}}>×</button>
                                </div>
                              ))}
                            </div>
                          )}
                          ...
                          <div style={{display:'flex',gap:6,marginBottom:8}}>
                            <input
                              ...
                              style={{flex:1,background:D4,border:'1px solid #333',borderRadius:7,color:'#E8E8E0',fontSize:12,padding:'7px 10px',outline:'none'}}/>
                            <button onClick={()=>addSubtask(t.id)} style={{background:'rgba(76,201,138,0.15)',border:'1px solid rgba(76,201,138,0.3)',color:'#4CC98A',borderRadius:7,padding:'7px 10px',cursor:'pointer',fontSize:11,fontWeight:600,whiteSpace:'nowrap'}}>+ Note</button>
                            <button onClick={()=>addBlocker(t.id)} style={{background:'rgba(201,76,76,0.12)',border:'1px solid rgba(201,76,76,0.3)',color:'#C94C4C',borderRadius:7,padding:'7px 10px',cursor:'pointer',fontSize:11,fontWeight:600,whiteSpace:'nowrap'}}>🚫 Block</button>
                          </div>
                          ...
                          <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:8}}>
                            ...
                            <input type="number" min="0" max="999" step="0.5" value={hours} placeholder="0"
                              ...
                              style={{width:52,background:D4,border:'1px solid #333',borderRadius:7,color:'#E8E8E0',fontSize:14,fontWeight:600,textAlign:'center',padding:'4px',outline:'none'}}/>
                          ...
                          <div style={{display:'flex',alignItems:'center',gap:8}}>
                            ...
                            <input type="date" value={deadline||''} onChange={e=>setDeadline(t.id,e.target.value)}
                              style={{background:D4,border:'1px solid #333',borderRadius:7,color:'#E8E8E0',fontSize:12,padding:'4px 8px',outline:'none'}}/>
                          </div>
                        </div>
                      )}
                    </div>
      ...
      {tab==='analytics'&&(
        <div style={{padding:'16px 20px'}}>
          ...
          <div style={{fontSize:10,letterSpacing:'2px',textTransform:'uppercase',color:G,opacity:.8,margin:'20px 0 12px'}}>Active blockers</div>
          {PHASES.flatMap(ph=>ph.tasks.map(t=>({t,ph,blockers:(state[t.id]?.subtasks||[]).filter(s=>s.type==='blocker'&&!s.done)}))).filter(x=>x.blockers.length>0).map(({t,ph,blockers})=>(
            <div key={t.id} style={{background:'rgba(201,76,76,0.08)',border:'1px solid rgba(201,76,76,0.2)',borderRadius:10,padding:'10px 14px',marginBottom:7}}>
              <div style={{fontSize:12,fontWeight:600,color:ph.color,marginBottom:6}}>{t.n}</div>
              {blockers.map(b=><div key={b.id} style={{fontSize:12,color:'#C94C4C',marginBottom:3}}>🚫 {b.text}</div>)}
            </div>
          ))}
          <div style={{fontSize:10,letterSpacing:'2px',textTransform:'uppercase',color:G,opacity:.8,margin:'20px 0 12px'}}>Overdue tasks</div>
